...
function PackagesFilterAndList() {
  ...
  const uniqueDestinations = ['all', ...Array.from(new Set(packages.map((p) => p.destination)))];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-slate-300 font-medium transition-colors"
            >
...
export default function PackagesPage() {
  return (
    <div className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <Suspense fallback={<div className="text-center py-12 text-slate-500">Loading search parameters...</div>}>
        <PackagesFilterAndList />
      </Suspense>
    </div>
  );
}
